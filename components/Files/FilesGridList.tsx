...
export default function FilesGridList({ files, onDelete, onProperties }: FilesGridListProps) {
  return (
    <PhotoProvider>
      <div className="flex flex-wrap gap-2 p-1.5">
        {files.map((f) => (
          <FileTile key={f.id} file={f} onDelete={onDelete} onProperties={onProperties} />
        ))}
      </div>
    </PhotoProvider>
  );
