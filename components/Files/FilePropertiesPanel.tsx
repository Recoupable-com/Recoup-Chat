...
export default function FilePropertiesPanel({ open, file, onClose, base }: FilePropertiesPanelProps) {
  const isDirectory = Boolean(file?.is_directory);
  const extension = !isDirectory && file?.file_name.includes(".")
    ...
    >
      <div className="p-4 h-full flex flex-col gap-4 w-80 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={onClose} aria-label="Close properties">
            <X className="h-4 w-4" />
          </Button>
        </div>
        ...
        {file ? (
          <div className="space-y-3 text-sm">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="col-span-1 text-xs text-muted-foreground">Type</div>
              <div className="col-span-2">{isDirectory ? "Folder" : file.mime_type || "File"}</div>
              {!isDirectory && (
                <>
                  <div className="col-span-1 text-xs text-muted-foreground">Extension</div>
