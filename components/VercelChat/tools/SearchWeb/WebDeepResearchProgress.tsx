...
export const WebDeepResearchProgress: React.FC<WebDeepResearchProgressProps> = ({ progress }) => {
  ...
  if (progress.status === 'searching') {
    ...
    return (
      <div className="border border-border dark:border-zinc-800 rounded-xl p-4 bg-muted dark:bg-zinc-900/50">
        <div className="flex items-baseline justify-between gap-4 mb-4">
          ...
          <span className="text-base font-semibold text-foreground dark:text-foreground whitespace-nowrap">
            {formatTimeRemaining(remainingSeconds)}
          </span>
        </div>

        <div className="relative h-1.5 bg-muted dark:bg-zinc-700 rounded-full overflow-hidden">
          <div 
