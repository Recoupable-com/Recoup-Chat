...
const StarterAgentCard: React.FC<StarterAgentCardProps> = ({ agent, onClick }) => {
  return (
    <button
      type="button"
      className="w-full bg-white border border-gray-200/60 rounded-lg px-4 py-3 hover:border-border hover:shadow-md transition-all duration-200 text-left group hover:-translate-y-px"
      onClick={() => onClick(agent)}
    >
      <div className="text-sm font-semibold text-gray-900 leading-tight group-hover:text-foreground transition-colors">
        {agent.title}
      </div>
